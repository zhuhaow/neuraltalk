...
from imagernn.data_provider import get_data_provider
from imagernn.solver import Solver
from imagernn.imagernn_utils import decode_generator, eval_split


def build_word_vocabulary(sentence_iterator, word_count_threshold):
...
def main(params):
    ...
    data_provider = get_data_provider(dataset)
    ...
    misc['wordtoix'], misc['ixtoword'], bias_init_vector = build_word_vocabulary(data_provider.iter_sentences('train'),
    ...
    def costfun(batch, model):
        ...
        return rnn_generator_cost(batch, model, params, misc)

    # calculate how many iterations we need
    num_sentences_total = data_provider.get_split_size('train', ofwhat='sentences')
    ...
    for it in xrange(max_iters):
        ...
        batch = [data_provider.sample_image_sentence_pair() for i in xrange(batch_size)]
        ...
        if (((it + 1) % eval_period_in_iters) == 0 and it < max_iters - 5) or is_last_iter:
            val_ppl2 = eval_split('val', data_provider, model, params, misc)  # perform the evaluation on VAL set
