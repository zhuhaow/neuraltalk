...
def main(params):
    ...
    dp = get_data_provider(dataset)

    misc = {'wordtoix': checkpoint['wordtoix']}
    ixtoword = checkpoint['ixtoword']

    blob = {'params': params, 'checkpoint_params': checkpoint_params,
            'imgblobs': []}  # output blob which we will dump to JSON for visualizing the results

    # iterate over all images in test set and predict sentences
    BatchGenerator = decode_generator(checkpoint_params)
    n = 0
    all_references = []
    all_candidates = []
    for img in dp.iter_images(split='test', max_images=max_images):
        n += 1
        ...
        img_blob = {'img_path': img['local_file_path'], 'imgid': img['imgid']}  # we will build this up
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', '--dump_folder', type=str, default="",
                        help='dump the relevant images to a separate folder with this name?')
