...
class BasicDataProvider:
    ...
    def get_split_size(self, split, ofwhat='sentences'):
    ...
    def sample_image_sentence_pair(self, split='train'):
        ...
        out = {'image': self._getimage(img), 'sentence': self._getsentence(sent)}
        return out

    def iter_image_sentence_pair(self, split='train', max_images=-1):
    ...
    # does not always return the same size batch
    def iter_image_sentence_pair_batch(self, split='train', max_images=-1, max_batch_size=100):
        batch = []
        for i, img in enumerate(self.split[split]):
            if i >= max_images >= 0:
                break
    ...
    def iter_images(self, split='train', shuffle=False, max_images=-1):
